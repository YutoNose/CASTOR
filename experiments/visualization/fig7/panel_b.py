...
def draw(ax, df=None, sample_id=None):
    from fig7.combined import _find_results_dir, _load_best_sample, _draw_volcano
    import pandas as pd
    ...
    _draw_volcano(ax, df, sample_id)


def create(sample_id=None):
    from fig7.combined import create_panel_b
    return create_panel_b(sample_id)


def main():
    fig = create()
    save_figure(fig, 'fig7/panel_b')
    plt.close(fig)
    print("Figure 7 panel B complete.")
